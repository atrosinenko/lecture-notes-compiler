...
Для запуска обработки нужно в терминале запустить run.py следующим образом:
python run.py <путь-к-каталогу-с-проектом> <выходное-имя-без-расширения>
Например:
python C:\konsp\run.py prj\alg algebra
...
* chop-background -- цвет фона, передаваемый команде convert. Поля этого цвета будут
    обрезаться.
...
* chop-edge (None)-- укажите для отсканированных страниц, если страница прижата
    ...
    значение None).
* chop-size -- ширина игнорируемой полоски
...
файл run.py, также считывается файл project.ini (если есть) из каталога проекта.

Для каждой секции вида "[__pluginname__]" загружается плагин pluginname.py из
каталога, содержащего main.py. Загрузка плагинов возможна только из config.ini.
